...
Type Parameter `Key` is corresponded to errorStateId's value in option which is second arguments of watchErrorsReducer. this API chains a reducer function to validation callback and if the result's not valid rollback state the previous one with save error object to internal state along configuration.

## watchErrorsReducer(reducer:Reducer<State,action:Action>,options?:{errorStateId?:string = 'errors',returnType?:string = 'object'}):Reducer(state:State&Record<Key, Error[]>,action:Action)

This API writes errors of validate methods to redux state, the depth can be arbitrary as how deep you apply watchErrorsReducer to selected reducer . This must call after all validate methods in the watchErrorsReducer applied to reducer's scope.This means we must not call watchErrorsReducer to get errors of parent's reducers or higher than them.This API must use to watch about child reducers or the reducer itself whether validation methods find errors, otherwise it is possible to start flushing errors to state though some validate method still aren't called  depends on the order of function invokes. Imagine the tree ,this derived one root node has many child nodes and decendants and depth-first search recuring process. Imagine one of these nodes is reducer,and the reducer executes fastest than the chirdlen and return last its'state. The callback set by watchErrorsReducer run after the reducer's return values, so the reducer can watch about all decendants and itself whether catch errors, therefore first origin node reducer is able to watch all of reducers it includes as decendants. So I highly recomend to control error states at top reducer.
...
## getInstance():{watchErrorsReducer,withValidateReducer}
...
const rootReducer = watchErrorsReducer(_validateReducer, {
  errorStateId: "hoge"
});
...
```typescript
const rootReducer = watchErrorsReducer(
    combineReducers({
